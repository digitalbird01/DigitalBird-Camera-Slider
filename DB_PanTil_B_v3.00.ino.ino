//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*Digital Bird Balanced PanTilt Head part of the Digital Bird Motion Control System Created by Colin Henderson 2021
...
String DBFirmwareVersion = "frm V:3.00";      //Current firmware version
...
void PTZ_MoveP() {
  ...
  if (stepper2->getCurrentPosition() != (P_position) && (P_position != 10)) {    //Run to  position
    ...
    stepper1->setSpeedInHz(4000);                           //Tilt
    stepper1->setAcceleration(2000);

    stepper2->setSpeedInHz(5000);                           //Pan
...
//**********************************************Stop Motion*******************************************
void Start_7() {
  if (Sld == 0) {                                                   //If no slider take charge of the count
...
void PTZ_Control() {
  ...
  if (abs(Joy_Pan_Speed) > 150 ) {
    pan_is_moving = true;
    stepper2->setAcceleration(abs(Joy_Pan_Accel) * 3);                            //If starting fresh Max accel to catch up on turnarround
    ...
    if (pan_is_moving) {
      stepper2->setAcceleration(abs(Joy_Pan_Accel) * 3);                                       //Breaking acceloration on the stick moving back to 0
      stepper2->applySpeedAcceleration();
      stepper2->stopMove();
      pan_is_moving = false;
    }
  ...
  if (abs(Joy_Tilt_Speed) > 250) {
    ...
    if (tilt_is_moving) {
      stepper1->setAcceleration(abs(Joy_Tilt_Accel * 3));                                         //Breaking acceloration on the stick moving back to 0
      stepper1->applySpeedAcceleration();
      stepper1->stopMove();
