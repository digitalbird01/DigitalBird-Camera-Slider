//////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*Digital Bird Balanced PanTilt Head part of the Digital Bird Motion Control System Created by Colin Henderson 2022
...
int Pan_J_Speed = 200;                        //Set this to 200 for the Balanced PT 4000 for the Over the Top PT
//********************************************************************************************************************
...
int  TLTTps_step_dist;
int  PANTps_step_dist;
int  FOCTps_step_dist;
...
void receiveCallback(const uint8_t *macAddr, const uint8_t *incomingData,  int Len) {
  ...
  inpress = int(incomingValues.InP);            //Inpoint button state 0-2
  outpress = int(incomingValues.OutP);          //Outpoint button state 0-2
...
void SendNextionValues() {
  ...
  NextionValues.InP = inpress;              // Inpoint button state 0-2
  NextionValues.OutP = outpress;            // Outpoint button state 0-2
  //NextionValues.Sld = Slider;             // Slider present 0-1
...
void Start_4() {
  ...
  TLTTps_step_dist = TLTtravel_dist / Tps;
  PANTps_step_dist = PANtravel_dist / Tps;
  FOCTps_step_dist = FOCtravel_dist / Tps;
  ...
  while (Tps >= 1) {
    ...
    if (Slider == 1) {                                                         //If Slider is conected give slider control over timing
      ...
      if (TpsM == 4) {                                                         //Trigger pantilt head timelapse move
        ...
        TpsM = 0;
        stepper1->moveTo(stepper1->getCurrentPosition() + TLTTps_step_dist);    //Current position plus one fps move
        delay(5);
        stepper2->moveTo(stepper2->getCurrentPosition() + PANTps_step_dist);    //Current position plus one fps move
        delay(5);
        stepper3->moveTo(stepper3->getCurrentPosition() + FOCTps_step_dist);    //Current position plus one fps move
    ...
    if (Slider == 0) {                                                        //If Slider is not with us take control
      Serial.println("Tps Trigger from PT");
