...
////Ps3 variables
////#define TT_ACCELERATION 8000
////#define TT_SPEED 2500
//unsigned long previousMillis = 0;
//const long debouncedelay = 100;
//float ttspeed_current = 0;
//float speedfactor = 1;
...
int JB = 1;                                          //This is the jib so tell the other parts you are present
//Sequencer
int SEQ = 0;
...
void receiveCallback(const uint8_t *macAddr, const uint8_t *incomingData,  int Len) {
  ...
  //JB = int(incomingValues.JB);                //Jib available 1 or 0 not used by JIb itself
...
void Start_3() {
  ...
  if (TT == 0 && PT == 0) {                               //Only take charge of the counting if no Pan head or PanTilt head are present on the jib
    Bounce = Bounce - 1;
...
void Start_4() {
  ...
  while (Tps >= 1) {
    TpsM = 4;                                                            //Trigger pant head timelapse move
    SendNextionValues();
    TpsM = 0;                                                             //Turn off trigger
...
void Start_7() {
  ...
  if (SMC >= 0  && stopM_cancel != 1) {
    TpsM = 4;                                                             //Trigger pan head move
    SendNextionValues();
    stepper1->moveTo(stepper1->getCurrentPosition() + Tps_step_dist);     //Current position plus one fps move
    while (stepper1->isRunning()) {                                       //delay until move complete (blocking)
      delay(10);
    }
    delay (3000);                                                         //Take a second to ensure camera is still
    digitalWrite(CAM, HIGH);                                              //Fire shutter
